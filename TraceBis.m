...
while k<dimPts(1) %les points sont déjà classés dans pts3D donc on trace un segment deux à deux
    ...
    elseif (pt2(2)~=pt1(2))
        ny=abs(pt2(2)-pt1(2));
        y=round(linspace(pt1(2),pt2(2),ny));
        for i=1:size(y,1)
            if(y(i)>0 && y(i)<dimFrame(1))
                frame(y(i),pt1(1),1) = 220;
                frame(y(i),pt1(1),2) = 20;
                frame(y(i),pt1(1),3) = 60;
            end
            if y(i)<dimFrame(1)
                    frame(y(i)+1,pt1(1),1) = 220;
                    frame(y(i)+1,pt1(1),2) = 20;
                    frame(y(i)+1,pt1(1),3) = 60;
            end
            if y(i)>1
                frame(y(i)-1,pt1(1),1) = 220;
                frame(y(i)-1,pt1(1),2) = 20;
